...
namespace IdeaStatiCa.Api.Common
{
	...
	public class HttpClientWrapper : IHttpClientWrapper
	{
		...
		public async Task<TResult> PostAsync<TResult>(string requestUri, object requestData, CancellationToken token, string acceptHeader = "application/json", bool useHeartbeatCheck = false)
		{
			...
			{
				var hubUrl = new Uri(baseUrl, RestApiConstants.RestProgressEndpoint);

				hubConnection = new HubConnectionBuilder()
					.WithUrl(hubUrl)
					.Build();

				hubConnection.On<string, int>(RestApiConstants.RestProgressMethod, (msg, num) => ProgressLogAction(msg, num));
