...
| [**GetTemplatesForConnectionAsync**](TemplateApi.md#gettemplatesforconnectionasync) | Retrieves a list of templates associated with a specific connection within a project. |
...
<a id="gettemplatesforconnection"></a>
## **GetTemplatesForConnectionAsync**
> **List&lt;ConConnectionTemplateModel&gt; GetTemplatesForConnectionAsync (Guid projectId, int connectionId)**
...
namespace Example
{
    public class GetTemplatesForConnectionAsyncExample
    {
        public static async Task Main()
        {
            string ideaConFile = "testCon.ideaCon";
            ...
            using (var clientFactory = new ConnectionApiServiceRunner(ideaStatiCaPath))
            {
                using (var conClient = await clientFactory.CreateApiClient())
                {
                    ...
                    {
                        // Retrieves a list of templates associated with a specific connection within a project.
                        List<ConConnectionTemplateModel> result = await conClient.Template.GetTemplatesForConnectionAsync(projectId, connectionId);
                        Debug.WriteLine(result);
                    }
                    catch (ApiException  e)
                    {
                        Console.WriteLine("Exception when calling Template.GetTemplatesForConnectionAsync: " + e.Message);
                        Console.WriteLine("Status Code: " + e.ErrorCode);
                        Console.WriteLine(e.StackTrace);
                    }
                    finally
...
[!code-csharp[](../examples/CodeSamples/Samples/GetTemplatesForConnection.cs)]
...
#### Using the GetTemplatesForConnectionWithHttpInfo variant
This returns an ApiResponse object which contains the response data, status code and headers.
...
{
    // Retrieves a list of templates associated with a specific connection within a project.
    ApiResponse<List<ConConnectionTemplateModel>> response = conClient.Template.GetTemplatesForConnectionWithHttpInfo(projectId, connectionId);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
...
{
    Debug.Print("Exception when calling TemplateApi.GetTemplatesForConnectionWithHttpInfo: " + e.Message);
    Debug.Print("Status Code: " + e.ErrorCode);
    Debug.Print(e.StackTrace);
}
```
