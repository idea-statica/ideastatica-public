...
| [**DeleteMemberAsync**](MemberApi.md#deletememberasync) | Deletes the specified member from the connection. |
...
<a id="addmember"></a>
## **AddMemberAsync**
> **ConMember AddMemberAsync (Guid projectId, int connectionId, ConMember conMember = null)**
...
| **conMember** | [**ConMember**](ConMember.md) | The member data for the new member. | [optional]  |
        ...
        public static async Task Main()
        {
            ...
            using (var clientFactory = new ConnectionApiServiceRunner(ideaStatiCaPath))
            {
                using (var conClient = await clientFactory.CreateApiClient())
                {
                    ...
                    connectionId = 56;  // int | The ID of the connection to add the member to.
                    var conMember = new ConMember(); // ConMember | The member data for the new member. (optional) 

                    try
                    {
                        // Adds a new member to the connection.
                        ConMember result = await conClient.Member.AddMemberAsync(projectId, connectionId, conMember);
                        Debug.WriteLine(result);
                    }
                    catch (ApiException  e)
                    {
                        Console.WriteLine("Exception when calling Member.AddMemberAsync: " + e.Message);
...
[!code-csharp[](../examples/CodeSamples/Samples/AddMember.cs)]
...
{
    // Adds a new member to the connection.
    ApiResponse<ConMember> response = conClient.Member.AddMemberWithHttpInfo(projectId, connectionId, conMember);
...
{
    Debug.Print("Exception when calling MemberApi.AddMemberWithHttpInfo: " + e.Message);
...
<a id="deletemember"></a>
## **DeleteMemberAsync**
> **int DeleteMemberAsync (Guid projectId, int connectionId, int memberId)**
...
|------|------|-------------|-------|
| **projectId** | **Guid** | The unique identifier of the opened project in the ConnectionRestApi service. |  |
| **connectionId** | **int** | The ID of the connection containing the member to delete. |  |
...
namespace Example
{
    public class DeleteMemberAsyncExample
    {
        public static async Task Main()
        {
            string ideaConFile = "testCon.ideaCon";
            ...
            using (var clientFactory = new ConnectionApiServiceRunner(ideaStatiCaPath))
            {
                using (var conClient = await clientFactory.CreateApiClient())
                {
                    ...
                    // (Required) Select parameters
                    connectionId = 56;  // int | The ID of the connection containing the member to delete.
                    memberId = 56;  // int | The ID of the member to delete.

                    try
                    {
                        // Deletes the specified member from the connection.
                        int result = await conClient.Member.DeleteMemberAsync(projectId, connectionId, memberId);
                        Debug.WriteLine(result);
                    }
                    catch (ApiException  e)
                    {
                        Console.WriteLine("Exception when calling Member.DeleteMemberAsync: " + e.Message);
                        Console.WriteLine("Status Code: " + e.ErrorCode);
                        Console.WriteLine(e.StackTrace);
                    }
                    finally
...
[!code-csharp[](../examples/CodeSamples/Samples/DeleteMember.cs)]
...
{
    // Deletes the specified member from the connection.
    ApiResponse<int> response = conClient.Member.DeleteMemberWithHttpInfo(projectId, connectionId, memberId);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
...
{
    Debug.Print("Exception when calling MemberApi.DeleteMemberWithHttpInfo: " + e.Message);
    Debug.Print("Status Code: " + e.ErrorCode);
    Debug.Print(e.StackTrace);
}
```
