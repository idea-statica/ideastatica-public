...
# X joint only
#   (two coaxial braces on OPPOSITE sides of the chord; the axial force of one
#    brace is transferred straight through the chord to the other)
#
# Console version (same principle as the T/Y loop script):
...
# - Plots resistances / utilisation vs theta
# - Saves the final result values to JSON
#
# X-joint specifics (vs. the T/Y script):
#   * X axial TENSION      Qu = 6.4 * gamma^(0.6*beta^2)          (Table 6-3, X)
#   * X axial COMPRESSION  Qu = (2.8 + (12 + 0.1*gamma)*beta) * Qbeta
#                          -> uses the GEOMETRIC factor Qbeta (X joints have no
#                             gap, so Qg does NOT apply).
#   * In-plane / out-of-plane bending Qu are the SAME for all classes.
#   * Table 6-4 Qf coefficients for X depend on BOTH the axial force sign
#     (tension / compression) AND on beta, with LINEAR interpolation between
#     beta = 0.9 and beta = 1.0 (note 1 under Table 6-4).
#
...
My_Sd = 25.0e6      # in-plane bending moment [N*mm]
Mz_Sd = 10.0e6      # out-of-plane bending moment [N*mm]
...
# -----------------------------------------------------------------------------
# Helper: Table 6-4 beta interpolation for X joints (note 1)
...
fig.suptitle("NORSOK N-004 6.4.3 - Simple X joint", fontsize=14)
...
# Write the JSON next to THIS script, not in the current working directory,
# so the file always lands in the X CONNECTION folder regardless of how the
# script is launched.
script_dir = os.path.dirname(os.path.abspath(__file__))
json_path = os.path.join(script_dir, "norsok_X_results_only.json")
