# NORSOK Checker — Python → C# unification (porting and verification context)
...
(`app.py` + `norsok/extract.py` + `norsok/n64.py`, plus the standalone per-joint
hand verification scripts). The goal of this branch: fold all verified python §6.4 logic into
the C# app so there is **one shipping tool**, with the python kept only as the
verification oracle.

## What we found before porting

The python §6.4 was trustworthy: `n64.py` and the hand verification scripts were written independently
...
1. **Formula engine** — 11 tests pinned to numbers produced by *running* the python:
   the `n64.py` self-test and the K / X / six-T-Y hand verification scripts (e.g. K util 0.3011,
...
3. **Live end-to-end** — `LiveValidationTests` (`[Explicit]`) opens the four benchmark
   ...
   | `X_CONNECTION` | CON1 0.620, CON3 **1.3130 FAIL** (= the X hand verification script), CON2 1.938 FAIL |
...
1. **CHS names with comma separators** — the benchmark files name sections
   ...
   so every member read as “not CHS” and the whole joint failed the gate. This was the
   open issue handed over with the verification scripts (commit `da19427f`) — fixed on **both** sides
...
- **Reference (do not extend)**: `reference/python_prototype/` and
  `reference/verification_scripts/` (the hand verification scripts + benchmark `.ideaCon`).
...
- **Tests**: `UT_NorsokChecker` — 452 discovered; **433 run offline** in the plain suite (~10 s),
  ...
  default into a folder that holds deliverables — it used to, and an ordinary test run then
  replaced the reviewer's copy. **`Category` selects; only `Explicit` deselects** — a probe needs
...
- **Engineering review** of the §6.4 report derivation blocks (per-class Qu/Qf table,
  K-per-gap, chord-stress trail — wording/units/conventions). The σ_mz sign question raised
  during the hand verification is numerically immaterial (σ_mz enters Qf only squared in A²,
  and Eq 6.57 uses |M_op|) and is closed in
  `reference/verification_scripts/CHORD_MOMENT_SIGNS.md`.
- **Section-name parsing**: the comma-name CHS issue is fixed as described above; the parser
  should be checked against any other section-naming convention that turns up (see
  `PYTHON_STOPGAP.md` — the dimensions are now read from the model, not the name).
