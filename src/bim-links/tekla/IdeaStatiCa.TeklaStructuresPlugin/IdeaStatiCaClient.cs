...
namespace IdeaStatiCa.TeklaStructuresPlugin
{
	public class IdeaStatiCaClient
	{
		...
		private IContainer BuildContainer()
		{
			...
			builder.RegisterType<ModelClient>().WithParameter(new TypedParameter(typeof(Tekla.Structures.Model.Model), new Tekla.Structures.Model.Model()));
		...
		public Task Run()
		{
			...
			string projectPath = CreateProjectDirectory(container.Resolve<ModelClient>());
		...
		{
			IContainer container = BuildContainer();
			var model = container.Resolve<ModelClient>();
			return model.GetProjectPath();
		}

		private static string CreateProjectDirectory(ModelClient model)
		{
