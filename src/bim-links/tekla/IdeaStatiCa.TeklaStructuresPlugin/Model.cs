...
namespace IdeaStatiCa.TeklaStructuresPlugin
{
	internal class Model : ICadModel
	{
		...
		public Model(ModelClient modelClient)
		{
			_model = modelClient;
		}
		...
		private CadUserSelection GetCadUserSelection(Point point, IEnumerable<TS.ModelObject> members, IEnumerable<TS.ModelObject> selectedObject)
		{
			...
			selectedObject.ForEach(teklaObject => identifiers = IdentifierHelper.GetIdentifier(_model,/* _model.GetItemByObjectId(sObj)*/teklaObject, ref identifiers));

			members.ForEach(teklaObject =>
			{
				if (teklaObject is TS.Beam beam)
				{
					identifiers = IdentifierHelper.GetIdentifier(_model, beam, ref identifiers, null, false, point);
				}
			});
