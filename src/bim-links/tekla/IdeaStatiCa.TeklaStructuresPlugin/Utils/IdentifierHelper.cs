...
namespace IdeaStatiCa.TeklaStructuresPlugin.Utils
{
	...
	{
		internal static List<IIdentifier> GetIdentifier(ModelClient model, ModelObject teklaObject, ref List<IIdentifier> identifiers, string constructionObjectHandle = null, bool addToCollection = true, Point connectionPoint = null)
		{
			...
			if (teklaObject is Part teklaPart)
			{
				...
				while (beamChildrenEnumerator.MoveNext())
				{
					...
					if (teklaChildObject is CutPlane cutPlane)
					{
						...
						identifiers = GetIdentifier(model, teklaChildObject, ref identifiers, constructionObjectHandle, addToCollection, connectionPoint);
					}
				...
				while (bolts.MoveNext())
				{
					...
					identifiers = GetIdentifier(model, modelObj, ref identifiers, constructionObjectHandle, addToCollection, connectionPoint);
				}
